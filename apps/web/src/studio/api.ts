...
async function call<T>(path: string, init: { method?: string; json?: unknown } = {}): Promise<T> {
  ...
  // Never waits for ever: a request stuck while the API restarts would leave every button waiting behind it.
  const send = (authorization: string) => fetch(`${API}/v1/studio/${path}`, {
...
export async function upload(set: string, key: string, file: File, kind: 'sketch' | 'final' | 'frame', note: string,
  ...
  return new Promise((resolve, reject) => {
    ...
    // An upload that stops moving for a minute (the connection dropped, the API restarting) is given up on and said
    // so, rather than left at the same percentage for ever. The file is still on the artist's computer.
    let stalled = window.setTimeout(() => xhr.abort(), 60_000);
    const moved = () => { window.clearTimeout(stalled); stalled = window.setTimeout(() => xhr.abort(), 60_000); };
    xhr.upload.onprogress = (e) => { moved(); if (e.lengthComputable) progress(e.loaded / e.total); };
    xhr.onloadend = () => window.clearTimeout(stalled);
    xhr.onabort = () => reject(new ApiError(0, 'offline'));
    xhr.onload = () => {
...
export function imageUrl(set: string, key: string, version: string): Promise<string> {
  ...
  if (!url) {
    url = (async () => {
      const auth = await authorization();
      const res = await fetch(`${API}/v1/studio/${set}/pictures/${key}/${version}`, {
        headers: auth ? { Authorization: auth } : {}, signal: AbortSignal.timeout(60_000),
      });
