...
| **Online games** (`fruitcats-api`: CPU, bandwidth, table writes) | Online play can't grow past its fixed room (`LIVE_MAX_PLAYERS`, 300 by default): players beyond it wait in line. To stop it altogether: `fruitcats-api` → **Environment variables** → add `LIVE` = `off` → Apply (the app restarts, about 2 minutes). Or lower `LIVE_MAX_PLAYERS`. | With `LIVE=off`, the Friend tile says "Paused for now" and games going end; everything else works. With fewer players allowed, more wait in line (buyers first). | Remove `LIVE`, or raise `LIVE_MAX_PLAYERS` again. |
