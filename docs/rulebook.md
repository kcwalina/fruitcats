...
| **Compost** | Your discard pile, face-up. Anyone may look through it. |
...
Both players do this at the same time. (If order ever matters, the Yarn Ball holder goes first.)

### 6.2 Action Phase

Starting with the player holding the Yarn Ball, players **alternate taking one action at a time**. On your action, choose one:
...
3. **The Yarn Ball rolls:** if nobody Took the Yarn this round, pass the Yarn Ball to the other player.

Then start the next round.

---

## 7. Pounce — Reacting to Your Opponent

Some Tricks and abilities have the keyword **Pounce**. They let you interrupt.

- When your opponent **plays a card** or **declares an attack**, you get one **Pounce window**: you may play **one** Pounce card (paying its cost as usual).
- Your Pounce resolves **first**. Then your opponent's card or attack resolves, if it still can.
- **You cannot Pounce on a Pounce.** One reaction, and that's it. There is no stack to learn.
- Pouncing does not use up your action; it's still your turn to act afterwards.
- A Pounce card may also be played as a normal action on your own turn.

> **Tip:** Treats you leave ready are a threat. Your opponent can't know if you're holding a Pounce.
...
When you lose a Life, take the **top card of your Lives** and put it **into your hand**. Getting hit hurts — but it also gives you more cards to fight back with.

### 9.2 Lucky

If the Life card you just took has the **Lucky** keyword, you may immediately reveal it and **play it for free**. (If you'd rather keep it in hand, you may.) A Lucky play cannot be Pounced on.

### 9.3 Winning

When a player loses their **ninth and final Life**, they lose the game immediately.
...
- **Defeat** — put a unit into its owner's Compost.
- **Summon** — put a token unit into your Yard (exhausted, unless it has Zoomies).
- **This round** — until the End Phase.

---

## 11. Building Decks & Competitive Play
...
**End Phase.** Catnip's bonus would wear off (the Hedgehog is already gone). Ben took the Yarn, so it doesn't roll. On to round 4.

---

## 13. Comprehensive Rules

These rules are written to be implemented exactly. Numbered for reference.
...
- **100.4** The only sources of randomness are shuffling a deck and choosing the starting Yarn Ball holder. No effect may use any other random process.
- **100.5** Hidden information: a player's hand, all face-down Life cards, deck order, and the opponent's face-down Treats. All else is public, including the number of cards in every zone.
...
- **300.2** *Start Phase* (skipped in round 1). In order: (a) each player readies all cards they control; "once per round" abilities reset; (b) "at the start of the round" triggers resolve, Yarn Ball holder's first; (c) the Yarn Ball holder draws 2, then the other player draws 2; (d) the Yarn Ball holder may plant one card, then the other player may plant one card. In digital play (d) is simultaneous and hidden; this is equivalent because planted cards are face-down.
- **300.3** *Action Phase.* The Yarn Ball holder has the first action. Players alternate actions. The phase ends when both players pass consecutively.
- **300.4** A player who has Taken the Yarn this round automatically passes whenever they would take an action. They may still use Pounce windows.
- **300.5** Take the Yarn may be performed by at most one player per round. The player may perform it even if they already hold the Yarn Ball (to prevent it from rolling).
- **300.6** *End Phase.* In order: (a) "at end of round" triggers resolve, Yarn Ball holder's first; (b) "this round" effects end; (c) each player with more than 10 cards in hand discards down to 10; (d) if no player Took the Yarn this round, the Yarn Ball passes to the other player.

### 400. Actions

- **400.1** *Play a card:* (a) reveal the card and choose all targets and modes; (b) exhaust ready Treats equal to its cost; (c) the opponent receives a Pounce window (500); (d) the card resolves: a unit enters the Yard exhausted (ready if it has Zoomies), then its Hello triggers; a Toy attaches to its chosen unit; a Trick performs its effect then goes to the Compost.
- **400.2** If, on resolution, a Trick or Toy has no legal target remaining, it does nothing and is put into the Compost. If only some targets are illegal, it affects the legal ones. Treats spent are not refunded.
- **400.3** *Use an ability:* pay the full cost, choose targets, resolve. Abilities do not open a Pounce window unless they declare an attack. An "Exhaust:" cost can be paid only by a ready card. A unit that entered the Yard exhausted cannot pay "Exhaust:" costs until readied.
...
- **500.1** A Pounce window opens for the non-acting player after costs are paid for a played card (400.1c) and after an attack is declared (600.2).
- **500.2** In a Pounce window the player may play one card with Pounce, or use one ability with Pounce, or decline. Costs are paid normally.
- **500.3** A card or ability played in a Pounce window resolves immediately and fully, including any triggers, before the original action continues. It does not itself open a Pounce window.
- **500.4** The Pouncing player knows the played card and its declared targets, or the attacker and its target.
- **500.5** Pouncing does not count as an action and does not count as passing or not passing for 300.3.
- **500.6** Cards played for free via Lucky (700.3) do not open a Pounce window.
...
- **700.1** When a player loses a Life, they put the top card of their Lives zone into their hand. If a player loses several Lives at once, they do so one at a time, completing 700.3 for each before the next.
- **700.2** A player with no cards in their Lives zone after losing a Life **loses the game** immediately (the final card still moves to hand first; Lucky is not applied).
...
- **700.4** If a player would draw from an empty deck, they lose 1 Life instead, once per card they could not draw.
...
| Lives per player | 9 |
| Deck size | 50 |
| Starting hand / Treats planted at setup | 6 / 2 |
| Cards drawn per Start Phase | 2 |
| Treats planted per Start Phase | up to 1 |
| Yard limit | 6 units |
| Maximum hand size at End Phase | 10 |
| Lives lost per hit / per Fierce hit | 1 / 2 |
