...
be shown in a circle, saved to art/avatars/<id>.webp at 512px. Every run costs money, so existing files are skipped.
"""
...
def main() -> int:
    ...
    parser.add_argument("--out", default="art/avatars", help="output folder, to try variants without replacing")
    args = parser.parse_args()

    out_dir = ROOT / args.out
    out_dir.mkdir(parents=True, exist_ok=True)
    todo = [k for k in AVATARS if (not args.only or k in args.only) and (args.force or not (out_dir / f"{k}.webp").exists())]
    if not todo:
