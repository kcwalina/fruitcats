...
function passing(e: unknown): boolean {
  const x = e as { statusCode?: number; code?: string; name?: string; message?: string; constructor?: { name?: string } };
  return (x.statusCode ?? 0) >= 500 || x.name === 'TimeoutError' || x.name === 'AbortError' || x.constructor?.name === 'CantCheckTokens'
