...
const catalog = buildCatalog(['DW1', 'SB1', 'HW1', 'BP1']);
...
describe('catalog', () => {
  ...
  it('never sells the starter set', () => {
    expect(catalog.sets).toEqual(['SB1', 'HW1', 'BP1']);
...
describe('deck prices: never pay twice', () => {
  ...
  it('brings every copy you are missing, but no starter cards you already have', () => {
    ...
    const gardens = fiveAlarm.cards['SB1-G01'];
    expect(missingForDeck(fiveAlarm, collectionOf({ 'SB1-G01': gardens }))['SB1-G01']).toBeUndefined();
  });
...
describe('a deck from a code, with cards you don’t have', () => {
  it('becomes the Store deck that brings them, plus the singles sold on their own', () => {
    const deck = { name: 'Hot Mess', hero: 'HW1-H01', cards: { 'HW1-P01': 3, 'HW1-P02': 3, 'HW1-X01': 1, 'DW1-D16': 3 } };
  ...
  it('lists cards the Store doesn’t sell apart', () => {
    const deck = { name: 'x', hero: 'DW1-H01', cards: { 'HW1-X01': 1 } };
  ...
  it('never sells an exclusive card, and says why', () => {
    ...
    try {
      ...
      const plan = cartForDeck({ name: 'x', hero: 'DW1-H01', cards: { 'HW1-X02': 1, 'HW1-X01': 1 } }, cat, starterOnly);
