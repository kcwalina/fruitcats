# Via Mochi accounts: how they work

This document describes the **Via Mochi account** as built and running: the one account system for Fruitcats (and
later other Via Mochi apps). It's the reference for how things work, where they live in Azure, and how to operate
them. What's still to do is in [accounts-plan.md](accounts-plan.md); the Store is in [store-plan.md](store-plan.md).
...
- **Code:** the account service is the private repository `viamochi-id` (proprietary, all rights reserved, like this one); deploy with its `deploy.ps1`.
...
| `purchases` (with the Store) | Every purchase, refund, chargeback, grant and revoke | **Forever**, tamper-proof and geo-redundant |

- **Enforced for free:** blob lifecycle rules per category, and immutability on `security` (1 year).
...
## Designed, not built: "Continue with Apple" and "Continue with Google"
...
- **Terms of Use** ([legal/terms-of-use.md](legal/terms-of-use.md)) and **Privacy Policy**
  ([legal/privacy-policy.md](legal/privacy-policy.md)), shown at `/terms.html` and `/privacy.html`. Drafts until the
  lawyer review. Each account records the Terms version it agreed to (see What's set up).
